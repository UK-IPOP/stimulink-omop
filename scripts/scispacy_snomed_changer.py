...
notes = pl.scan_csv(
    "~/068IPOP_STIMuLINK-DataAnalytics/DOFSS/DOFSS_StimuLink_extract_020723.csv"
).drop_nulls(subset=["NarrativeCME"])
document_data = [
    (
        row[1],  # note
    ...
    for row in tqdm(
        notes.select(["Incident_ID", "NarrativeCME"]).collect().to_numpy(),
        desc="Making tuples...",
    )
]
del notes
...
with open("snomed_output.jsonl", "wb") as f:
    ...
    for doc, context in tqdm(
        nlp.pipe(document_data, as_tuples=True, batch_size=1_000, n_process=4),
